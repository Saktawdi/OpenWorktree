...
/**
 * 轻量收敛：用服务端 session_todo 快照回算该会话的任务清单（不触碰聊天视图与
 * 上下文占用）。供回合结束与「无 SSE 直连」的后台会话轮询兜底——流式事件一旦
 * 丢失（断流/刷新/后台运行），任务清单在此与持久化快照最终一致，不再只靠
 * 手动切会话/切工单才会刷新。
 */
export async function syncSessionTodos(sessionId: string) {
  try {
    const d = await api<{ todos?: TodoItem[] }>(`/api/sessions/${sessionId}/todos`);
