    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5 px-5 h-12 border-b border-edge">
          ...
          <span className="text-[13.5px] font-semibold">{t("gate.history.title")}</span>
          <span className="flex-1" />
          <button className="icon-btn" onClick={() => openStageChangesView(null)} aria-label={t("common.close")}>
            ✕
          </button>
        </div>
                ...
                <div key={i} className="rounded-lg border border-edge bg-sunken/40 p-3.5">
                  <div className="flex items-center gap-2 text-[12px]">
                    <span className="chip border border-edge-strong bg-raised text-dim">
                      {stageChangeKindLabel(r.kind, t)}
                    </span>
                    <span className="chip border border-accent/30 bg-accent/10 text-accent font-mono">
                      {t("gate.history.round", { n: r.round })}
                    </span>
                    <span className="text-dim">{stageLabel(r.fromStage, t)}</span>
                    <ArrowRight size={11} className="text-faint" />
                    <span className="text-ink font-medium">{stageLabel(r.toStage, t)}</span>
                    <span className="flex-1" />
                    <span className="font-mono text-[10.5px] text-faint">
                      {r.createdAt ? new Date(r.createdAt).toLocaleString(getLocale()) : "—"}
                    </span>
                  </div>
