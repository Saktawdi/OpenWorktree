...
import { CaretDown, CaretUp, FileCode } from "@phosphor-icons/react";
import { NO_DIFF, useApp } from "../lib/store";
import type { DiffFile } from "../lib/types";
import { useState } from "react";
import { diffTotals } from "../lib/diff";

function FileBlock({ file, highlightLine }: { file: DiffFile; highlightLine?: number }) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-auto px-5 py-4">
      <div className="max-w-[900px] mx-auto space-y-3">
        ...
        {files.map((f) => (
          <FileBlock key={f.path} file={f} highlightLine={highlight?.path === f.path ? highlight.line : undefined} />
        ))}
      </div>
    </div>
  );
