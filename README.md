...
| `ticket_edit` | Agent | 编辑工单元信息：标题、优先级、需求描述、备注、标签。只改你传的字段（不传即保持原值），传空串（标签传 `[]`）即清空该可选字段；传 JSON `null` 等同不传。**没有 stage 参数**——工单状态由门禁流转（预提审 → 审查 → 发布）与人工重启/取消掌握，Agent 无从表达状态变更。作用域：Agent 只能改自己绑定的这张工单，`ticket_no` 可省（省即自己绑定的工单），指名别的工单一律拒绝；Human 不限 |
...
| `session_read` | Agent | 只读查阅会话内容：返回会话元信息（工单、状态、模型、用量）与按时间序的消息（角色、正文、思考、工具调用及其结果）。默认取最新 20 条（上限 100），每条带它在全量转录中的绝对序号，可把返回的 `next_before_index` 当作 `before_index` 往更早翻页；超长字段带显式截断标记、整份响应有体积上限，长会话请翻页读。作用域与 `ticket_create` 同源：Agent 只能读<b>自己项目</b>下的会话（同项目其它工单的会话可读，跨项目会话拒绝），Human 不限 |
...
- **首次启动引导**：第一次打开自动弹出七步配置向导（语言偏好 → 连接后端 → 接入项目 → 智能体设置 → LLM 设置 → 看板 → 工作台），可一键跳过，之后随时可在「设置中心 → 偏好 → 新手引导」重看。
...
### 方式一：Windows 桌面版与单文件原生版（推荐，双击即用）

同一个后端、两种落地形态：桌面版带安装界面、随系统托盘常驻（内嵌的正是下方单文件原生版那份二进制），原生版免安装、一条命令即起。日常使用推荐桌面版；服务器 / 免安装场景直接用原生版。
...
- 桌面壳内嵌的后端就是下方「单文件原生版」那份二进制，行为与源码方式完全一致。

#### 单文件原生版（Linux / Windows，无需 Java / Node）
...
> 镜像仍在发布，但**不推荐日常使用，云端部署更不推荐**：容器常驻 JDK + nginx + opencode，内存占用明显高于原生方式；只建议在临时体验、或本机完全没有环境的评估场景使用。
