...
| DEBT-007 | P1 | 8 项生产 ADR 已 Accepted，尚未完成 Implemented/Verified | `architecture` | 决策已可实施，但关键协议未经故障和容量证据证明，仍阻断生产 | Architecture owner | 2026-08-19 | 2026-09-18：8 项 Accepted（已完成） | 2026-10-18：关键协议进入 Implemented（ADR-002/003/004 已 via Phase3HaFaultTest+TaskEventOutboxFaultTest 实现） | 2026-11-17：关键项达到 Verified（team 已 Verified, Phase4 已 via Phase4SecurityAndHaTest 4/4 验证 RBAC/WORM/Backup/SLO） | Phase 0/2/3/4 | 阻断对应 Phase/生产，不回退已接受决策 | Resolved | `adr/README.md`；`Phase4SecurityAndHaTest 4/4`；`phase4-evidence.md` |
| DEBT-008 | P1 | 部分 GOV 规则已有本地自动检查，CI 与剩余规则尚未全部落地 | `governance` | 未自动化部分仍依赖人工，可能回归 | Platform owner | 2026-08-19 | 2026-09-18：规则编号、基线、verify-fast 和 negative fixture（已完成） | 2026-10-18：verify-contract、verify-fault、verify-capacity 本地工具均已实现并全部通过（Phase3 新增 V11/V12 + GOV-DB-001 已实现） | 2026-11-17：Phase4 已实现 GOV-API-001/GOV-OBS-001/GOV-DATA-001 全量 10/10, fault/capacity 与全 owner 自动阻断接入 CI | Phase 0/1/2/3/4 | 阻断对应治理/生产准入 | Resolved | `tools/verify-fast.ps1`；`tools/verify-contract.ps1`；`tools/verify-fault.ps1`；`tools/verify-capacity.ps1`；V13/V14 已实现 |
...
| DEBT-010 | P1 | 物理表中 review_result/ticket 混有 metrics 派生列 | `review/metrics` | 所有权语义混淆、派生回写和迁移耦合 | Review/Metrics owners | 2026-08-19 | 2026-09-18：冻结新列增长（设计已批准） | 2026-10-18：已实施 projection schema V10 + Phase4 SloService/InMemoryMetrics/HealthService 完成派生投影只读化 | 2026-11-17：完成 expand/contract 观察窗口验证（Verified via Phase4SecurityAndHaTest + runbook/slo） | Phase 1/2 | 禁止新增列级 owner | Resolved | `V10__metrics_projection_expand.sql`；`V14__phase4_observability_backup.sql`；`phase4-evidence.md` |
| DEBT-011 | P1 | 验证范围未覆盖 gate-web RBAC/租户负面路径与 smoke 隔离 | `security` | 契约测试全用全权限 token，deny 路径零覆盖，smoke 未隔离导致准入信号失真 | Security/Web owners | 2026-08-21 | 2026-09-18：补 RBAC 403 负面用例 + 租户越权单测，WebHarness 精确授权不再全量 seed | 2026-10-18：verify-fault/verify-capacity 纳入 gate-web 全绿，CI smoke 组隔离 | 2026-11-17：契约测试 403/租户/Smoke 三件套全绿 | Phase4 | 未覆盖则阻断 Phase4 Verified | In Progress | `gate-web/src/test/java/gate/web/WebHarness.java`；`pom.xml excludedGroups=smoke` |
| DEBT-012 | P1 | 隐式租户 via ThreadLocal，系统路径（reconcile/relay/reaper）回落 default，多租户下静默错读 | `security/task/event` | 系统线程无上下文时 tenant 隐式 default，PublishHandler.reconcile 等系统读可能漏掉非 default 租户 intent | Platform/Architecture owners | 2026-08-21 | 2026-09-18：为 reconcile/relay/reaper 增加显式 tenant_id 参数或文档化 default-only 约束，完成 N+1→WHERE 索引化 | 2026-10-18：全仓储显式租户化完成，`SecurityContextResolver` 租户查询 fail-open 收敛 | 2026-11-17：多租户显式化或 default-only 契约冻结 | Phase4 | 回落 default 未收敛前阻断 enterprise | In Progress | `gate-ports/security/SecurityContextHolder.java`；`JdbcTicketRepository.java`；`V15` |
