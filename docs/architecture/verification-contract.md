...
| `GOV-DATA-001` | `verify-contract --rule GOV-DATA-001` | ownership-catalog.md + migration parser | 未登记表/列/对象写入者 | 已实现；Phase4 ownership 全覆盖 V13/V14 + API/健康/SLO/备份表 |
| `GOV-API-001` | `verify-contract --rule GOV-API-001` | API/event schema baseline | breaking diff、版本和 ADR | 已实现；ApiRoutes 全量 owner 登记 + verify-contract 11 migrations scanned passed |
| `GOV-DB-001` | `verify-contract --rule GOV-DB-001` | Flyway migrations + compatibility matrix | 漂移、锁风险、expand/contract 违规 | 已实现；V11/V12 nonce+S3+PG兼容索引，V13/V14 RBAC/WORM/SLO/备份，verify-contract 14 migrations scanned passed |
...
| `GOV-OBS-001` | `verify-contract --rule GOV-OBS-001` | capability registry + telemetry catalog | 缺失指标/trace/error code/owner | 已实现；MetricsPort/TracingPort/SloService/HealthService + InMemoryMetrics + runbook/alert (§13-14) |
