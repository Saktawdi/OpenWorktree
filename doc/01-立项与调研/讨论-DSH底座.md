...
> 上游：`../02-执行文档/执行文档-后端-web.md`（S0–S5、ADR-10..14）、`立项讨论-v2.md`（§6.2 安全硬要求、§12 竞品定位、§13.1 M1 范围）、`../归档/done/*`（P0–P4 定案）
> 平行：`执行文档-前端-web.md`
...
1. **违背定案定位**：`../README.md` 冻结的定位是「**一个 CLI 无关的 git 提交闸门层**」。插件化把「适配任意 agent CLI」变成「只适配 DSH 生态」——CLI 无关是 v2 §12 检索后收敛出的差异点之一，不能自毁。
...
理由见 §4.4。批复后：`../02-执行文档/执行文档-后端-web.md` 维持原样（S0–S5 不变），本讨论归档为「评估记录 + 否决记录」。
...
| 阶段 | 内容 | 变化 |
|---|---|---|
| S0–S5 | 原计划不动（`../02-执行文档/执行文档-后端-web.md` / `执行文档-前端-web.md`） | 无 |
...
2. 批复后：`../02-执行文档/执行文档-后端-web.md` / `执行文档-前端-web.md` 从「待审查」转「定案」，按原计划开 **S0**（gate-web 骨架已有未提交脚手架，直接续做）；
...
*本文档为立项/执行系列的讨论稿 v2。v1（推荐插件化）因「市场受众 + CLI 无关定位」论证被否决，本文档取代 v1；事实清单（§1）与映射（§3）保留为评估记录。定案后按 §8 流程更新 `../README.md` 索引并归档。*
